...
converter.convertAsciiDoc = function (content, links, inline) {
  element.innerHTML = convertAsciiDoc(content, links || {}, inline);
  element.innerHTML = element.innerHTML.replace(/<p>\s*<\/p>/g, '');
  return element.innerHTML.replace(/\n\r?$/, '');
};
...
function convertAsciiDoc (content, links, insideContentClass) {
  var i, tag, asciidoc = '', html;
  ...
  for (i = 0; i < content.length; ++i) {
    if (typeof content[i] === 'string') {
      ...
      tag = content[i].block ? 'div' : 'span';
      asciidoc += '<' + tag + ' class="' + content[i].class + '">';
      asciidoc += convertAsciiDoc(content[i].content, links, true);
      asciidoc += '</' + tag + '>';
    }
  }
